...
public class MultiDirectionSlidingDrawer extends ViewGroup {
	...
	public MultiDirectionSlidingDrawer( Context context, AttributeSet attrs, int defStyle )
	{
		...
		TypedArray a = context.obtainStyledAttributes( attrs, vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer, defStyle, 0 );
		
		int orientation = a.getInt( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_direction, ORIENTATION_BTT );
		mVertical = ( orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB );
		mBottomOffset = (int)a.getDimension( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_bottomOffset, 0.0f );
		mTopOffset = (int)a.getDimension( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_topOffset, 0.0f );
		mAllowSingleTap = a.getBoolean( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_allowSingleTap, true );
		mAnimateOnClick = a.getBoolean( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_animateOnClick, true );
		mInvert = ( orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR );
		
		int handleId = a.getResourceId( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_handle, 0 );
		if ( handleId == 0 ) { throw new IllegalArgumentException( "The handle attribute is required and must refer "
				+ "to a valid child." ); }
		
		int contentId = a.getResourceId( vn.com.flex.dragonfly.R.styleable.MultiDirectionSlidingDrawer_content, 0 );
